			...
			public void run() {
				try {
					...
					runOnUiThread(new Runnable() {
						public void run() {
							progressDialog.dismiss();
							GreenToast.makeText(getApplicationContext(), "退出聊天室失败: " + e.getMessage(), 1).show();
						}
					});
				}
			}
...
//				Drawable avatar = getResources().getDrawable(R.drawable.default_avatar_ori);
					...
					public void onClick(View v) {
						if (isInDeleteMode) {
							...
							if (!NetUtils.hasNetwork(getApplicationContext())) {
								GreenToast.makeText(getApplicationContext(), getString(R.string.network_unavailable), 0).show();
								return;
							}
